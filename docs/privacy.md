...
When “Share anonymous usage and performance data” is enabled, Debut may send bucketed operation counts, latency ranges, workload class, a coarse first-use/cache-temperature class, app version, and macOS major version. Records contain no trace ID, exact phase timing, cache count, or persistent user or installation identifier. The initial transport is designed for TelemetryDeck's EU-hosted ingestion, with automatic capture, user identification, advertising, cross-app tracking, fingerprinting, and session replay disabled by construction.
