...
export SPARKLE_EDDSA_PRIVATE_KEY=nightly-key SPARKLE_PUBLIC_ED_KEY=nightly-public-key
"$validator" nightly
python3 - "$root" <<'PY'
...
assert 'SPARKLE_EDDSA_PRIVATE_KEY: ${{ secrets.SPARKLE_EDDSA_PRIVATE_KEY }}' in nightly
assert 'SPARKLE_PUBLIC_ED_KEY: ${{ vars.SPARKLE_PUBLIC_ED_KEY }}' in nightly
...
assert 'SPARKLE_EDDSA_PRIVATE_KEY: ${{ secrets.SPARKLE_EDDSA_PRIVATE_KEY }}' in check
assert 'SPARKLE_PUBLIC_ED_KEY: ${{ vars.SPARKLE_PUBLIC_ED_KEY }}' in check
assert '.build/appcast.xml' in check
