...
nonisolated(unsafe) var currentScenario = ""
nonisolated(unsafe) var currentGroup = ""
nonisolated(unsafe) var recordedScenarios: [[String: String]] = []
...
let checkResultsFile: URL = {
    ...
    let invocation = arguments.first.map { $0.hasPrefix("--") } ?? true
        ? "suite" : arguments.joined(separator: "-")
...
func recordCheck(_ name: String, status: String, reason: String? = nil) {
    ...
    if !currentScenario.isEmpty {
        check["scenario"] = currentScenario
        check["group"] = currentGroup
    }
...
/// A scenario's own outcome: ran, not_selected, or setup_failed when base state could not be
/// restored before it. Not selecting a scenario is recorded, never silently omitted.
func recordScenario(_ scenario: E2EScenarioDescriptor, status: String) {
    recordedScenarios.append(["id": scenario.id, "group": scenario.group.rawValue, "status": status])
    writeCheckResults()
}
...
func writeCheckResults() {
    ...
    guard let data = try? JSONSerialization.data(
        withJSONObject: [
            "invocation": invocation, "checks": recordedChecks, "scenarios": recordedScenarios,
        ],
...
if CommandLine.arguments.dropFirst().first == "--harness-self-check" {
    ...
    expect(parseE2EInvocation(["--groups", "drag-drop,smoke"])
               == .success(.suite(groups: [.dragDrop, .smoke])),
           "--groups keeps the requested order")
    expect(parseE2EInvocation(["--groups", "smoke,smoke"]) == .success(.suite(groups: [.smoke])),
           "a repeated group runs once")
    expect(parseE2EInvocation(["--list"]) == .success(.list), "--list lists")
    expect(parseE2EInvocation(["--plan", "--groups", "fullscreen"])
               == .success(.plan(groups: [.fullscreen])),
           "--plan names the selection without running it")
    expect((try? parseE2EInvocation(["--groups", "nonsense"])?.get()) == nil,
           "an unknown group is an error, not an empty run")
    expect((try? parseE2EInvocation(["--groups"])?.get()) == nil,
           "--groups needs a value")
    expect((try? parseE2EInvocation(["--groups", ""])?.get()) == nil,
           "a selection of nothing is an error")
    expect((try? parseE2EInvocation(["--frobnicate"])?.get()) == nil,
    ...
    expect(plannedScenarios(groups: [.windowMoves, .smoke]).map(\.group)
           ...
           "groups run in the requested order")
    expect(Set(e2eScenarioCatalog.map(\.group)) == Set(E2EGroup.allCases),
           "every group owns at least one scenario")
    expect(Set(e2eScenarioCatalog.map(\.id)).count == e2eScenarioCatalog.count,
...
// Suite options are settled before the screenshot directory is cleared or anything launches:
// listing, planning and bad options never touch the session.
let e2eInvocation: E2EInvocation? = {
    guard let parsed = parseE2EInvocation(Array(CommandLine.arguments.dropFirst())) else { return nil }
    switch parsed {
...
case .suite?:
    ...
    // disposable session may say it is one; the developer's own desktop never should.
    guard environment["DEBUT_E2E_DISPOSABLE_SESSION"] == "1" else {
        FileHandle.standardError.write(Data(("The E2E suite drives the whole session. Run it in the "
            + "headless Tart VM (scripts/tart-e2e.sh run), or set DEBUT_E2E_DISPOSABLE_SESSION=1 "
            + "only in a disposable session.\n").utf8))
        exit(2)
    }
case nil:
    break
...
if CommandLine.arguments.dropFirst().first == "onboarding-permission-check" {
    ...
    var passed = runtimeReady && appeared
    if deniedAccessibility {
        passed = passed && onboardingContains("Required") && !onboardingGetStartedEnabled()
        _ = takeScreenshot("onboarding_accessibility_denied")
    } else {
        passed = passed && onboardingPress("Get started") && onboardingContinueEnabled()
        if oneDesktop {
            passed = passed && SpaceService().userDesktops().count == 1
                && onboardingContains("Unlock more with multiple desktops")
            _ = takeScreenshot("onboarding_one_desktop")
        }
        passed = passed && currentOnboardingTarget() == nil
            && onboardingPress("Continue") && onboardingContinueEnabled()
            && onboardingPress("Continue") && onboardingContains("Enable faster desktop switching")
            && onboardingPress("Continue") && onboardingContains("You’re ready")
            && onboardingPress("Start using Debut")
        if oneDesktop { passed = passed && SpaceService().userDesktops().count == 1 }
        _ = takeScreenshot("onboarding_without_capture_complete")
    }
    _ = terminateDebutAndWait()
    let journey = oneDesktop ? "one-desktop" : deniedAccessibility ? "denied-accessibility" : "denied-capture"
    print("Onboarding \(journey) flow: \(passed ? "PASS" : "FAIL")")
    // The journeys are the permissions group; record them like any other check.
    currentGroup = "permissions"
    currentScenario = "onboarding-\(journey)"
    recordCheck("First-use onboarding completes with \(journey)", status: passed ? "passed" : "failed")
    exit(passed ? 0 : 1)
}
...
@MainActor
func scenario_system_overviews() {
    let userDesktopCount = SpaceService().userDesktops().count
...
@MainActor
func scenario_overlay_keyboard() {
...
@MainActor
func scenario_overlay_keyboard_move() {
...
@MainActor
func scenario_overlay_fullscreen() {
    ...
    if enteredFullscreen {
        // The Space animation keeps running after the attribute flips. Debut learns which window
        // has focus from that app's focus report, which a just-launched fixture or Debut can
        // deliver late; give it that long, and let the check below fail if it never arrives.
        wait(2)
        _ = waitFor(timeout: 5) { readState()["focusedWindowFullscreen"] == "true" }
        postFlagsChanged(flags: [.maskCommand])
        wait(0.1)
        postKeyDown(keyCode: CGKeyCode(kVK_Tab), flags: [.maskCommand])
        wait(1.5)
...
@MainActor
func scenario_custom_activation() {
...
func scenario_navigation_controls() {
    ...
    let commandTabBefore = eventCursor()
    postFlagsChanged(flags: .maskCommand)
    postKeyDown(keyCode: CGKeyCode(kVK_Tab), flags: .maskCommand)
    wait(0.3)
    test("Disabled workspace isolation leaves Command-Tab to macOS") {
        ...
        return state["eventTapRunning"] == "true"
            ...
            && events(since: commandTabBefore) { $0["keyEvent"] == "cmdTabHold" }.isEmpty
    }
    postKeyUp(keyCode: CGKeyCode(kVK_Tab), flags: .maskCommand)
    postFlagsChanged(flags: [])
    finishFeatureControls(settingsBackup: featureSettingsBackup)
}

@MainActor
func scenario_fullscreen_navigation() {
    header("Fullscreen Space navigation")
    let (featureReady, featureSettingsBackup) = launchWithFeatureControls()
    let featureSpaces = SpaceService()
    if featureReady, featureSpaces.userDesktops().count >= 2, quickSwitch(to: 0, using: featureSpaces) {
        ...
        if fullscreenNavigationEntered,
           ...
           stack.currentDesktopIndex == nil {
            ...
            if controlEvents < 2 {
                // Which press went unclaimed, and whether Debut saw it at all.
                for event in events(since: controlEventsBefore) {
                    let details = event.keys.sorted()
        ...
        test("Fullscreen navigation fixture has Debut ready on the first of two desktops") { false }
    }
    finishFeatureControls(settingsBackup: featureSettingsBackup)
}

@MainActor
func scenario_system_duration_transition() {
...
func onboardingButton(_ title: String, role wantedRole: String = kAXButtonRole) -> AXUIElement? {
    ...
    func find(_ element: AXUIElement) -> AXUIElement? {
        ...
        if role as? String == wantedRole {
            ...
            // A switch-style Toggle exposes its label as a child static text, not as its own
            // AXTitle, so a title-only match never finds one. Match the identifier as well.
...
func onboardingControls(role wantedRole: String) -> [String] {
    ...
    func visit(_ element: AXUIElement) {
        ...
        AXUIElementCopyAttributeValue(element, kAXRoleAttribute as CFString, &role)
        if role as? String == wantedRole {
            let names = [kAXTitleAttribute, kAXDescriptionAttribute, kAXIdentifierAttribute].map {
                var value: CFTypeRef?
                AXUIElementCopyAttributeValue(element, $0 as CFString, &value)
                return value as? String ?? ""
            }
            found.append(names.joined(separator: "/"))
        }
        var children: CFTypeRef?
        AXUIElementCopyAttributeValue(element, kAXChildrenAttribute as CFString, &children)
...
func onboardingPress(_ title: String, role: String = kAXButtonRole) -> Bool {
    guard let button = onboardingButton(title, role: role) else {
        info("no \(role) matching \(title); available: \(onboardingControls(role: role))")
        return false
    }
...
func loadOnboardingSettings() -> AppSettings? {
...
@MainActor
func scenario_onboarding_journey() {
    ...
    func performOnboardingExercise(_ practice: OnboardingPractice) -> Bool {
        ...
        case .allWindows: "Tutorial complete"
        }
        ...
        return waitFor(timeout: 8) {
            ...
            case .workspace:
                if SpaceService().userDesktops().count == 1 {
                    return currentOnboardingTarget()?["title"] == "Tutorial complete" && onboardingContains("Hold Option and press Tab")
                }
            ...
            case .moveWindow: return currentOnboardingTarget()?["title"] == "Tutorial complete" && onboardingContains("Hold Option and press Tab")
            case .allWindows: return onboardingContains("Tutorial complete")
            }
        }
    }
    ...
    test("Welcome combines the pitch with required and optional permissions") {
        onboardingContains("Turns the macOS Command-Tab switcher into a workspace manager")
            && onboardingContains("Accessibility") && onboardingContains("Screen Recording")
            && onboardingContains("Required") && onboardingContains("Optional")
    }
    test("Command-Tab setup continues without practice and persists its toggle") {
        guard onboardingPress("Get started"), onboardingContinueEnabled(), currentOnboardingTarget() == nil,
              !onboardingContains("Unlock more with multiple desktops"),
              onboardingPress("onboarding-command-tab", role: kAXCheckBoxRole) else { return false }
        let disabled = waitFor { loadOnboardingSettings()?.features.workspaceIsolation == false }
        return disabled && onboardingPress("onboarding-command-tab", role: kAXCheckBoxRole)
    }
    _ = takeScreenshot("11_setup_command_tab")
    test("Option-Tab setup exposes an independent switch") {
        guard onboardingPress("Continue"), onboardingContinueEnabled(),
              onboardingPress("onboarding-option-tab", role: kAXCheckBoxRole) else { return false }
        let disabled = waitFor { loadOnboardingSettings()?.features.optionTab == false }
        return disabled && loadOnboardingSettings()?.features.workspaceIsolation == true
    ...
    test("Faster desktop setup preserves method choices when toggling its master") {
        ...
        guard let after = loadOnboardingSettings() else { return false }
        return changed && before.features.numberShortcuts == after.features.numberShortcuts
            && before.features.controlArrows == after.features.controlArrows
    ...
    test("The optional tutorial starts only after setup completes") {
        guard onboardingPress("Continue"), onboardingContains("You’re ready"),
              onboardingButton("Open Settings") != nil, onboardingButton("Start using Debut") != nil,
              onboardingPress("Start tutorial") else { return false }
        return waitFor {
            readEvents().contains { $0["event"] == "onboarding_completed" }
    ...
    test("Closing the tutorial cleans up both windows and the menu resumes it") {
        guard let target = currentOnboardingTarget(),
              let lessonID = target["lessonWindowID"].flatMap(UInt32.init),
              let targetID = target["windowID"].flatMap(UInt32.init),
              AccessibilityWindowService().closeWindow(windowID: lessonID) else { return false }
        guard waitFor({
            let liveIDs = Set(AccessibilityWindowService().listWindows().map(\.windowID))
            return !liveIDs.contains(lessonID) && !liveIDs.contains(targetID)
        }), onboardingMainMenuPress("Tutorial...") else { return false }
        return waitFor {
            currentOnboardingTarget()?["lessonWindowID"] != String(lessonID)
    ...
    test("Option-Tab reaches the named target without a capture gate") { performOnboardingExercise(.allWindows) }
    _ = takeScreenshot("11_tutorial_complete")
    // The last exercise ends with a Space switch, and the tutorial's AX tree can be empty for a
    // moment afterwards although its buttons are already drawn. Wait for the control to exist
    // rather than read that lag as a missing button.
    _ = waitFor(timeout: 5) { onboardingButton("Finish tutorial") != nil }
    test("Finishing the tutorial closes it without reopening setup") {
        onboardingPress("Finish tutorial") && !onboardingContains("Tutorial complete")
            && !onboardingContains("Welcome to Debut")
    }
    test("Tutorial can be replayed and exited without changing settings") {
        guard let before = loadOnboardingSettings(), onboardingMainMenuPress("Tutorial..."),
              waitFor({ onboardingContains("Hold Command and press Tab") }),
              onboardingPress("Skip lesson"), onboardingPress("Exit tutorial") else { return false }
        return waitFor { !onboardingContains("Debut tutorial") }
            && loadOnboardingSettings() == before
    }
...
@MainActor
func scenario_selected_window_dismissal() {
    // --- 15. Selected window dismissal ---
    header("15. Selected window dismissal")

    // Borrowing the settings scenario's relaunch coupled this group to that one; launch fresh.
    _ = terminateDebutAndWait()
    clearDiagnosticFile()
    let dismissalApplication = launchDebut()
    _ = waitForDebutReady(dismissalApplication)
...
@MainActor
func scenario_recycled_window_id() {
...
@MainActor
func scenario_startup_discovery() {
...
@MainActor
func scenario_launch_focus_and_resize() {
...
@MainActor
func scenario_move_duration_sweep() {
    ...
    if moveSpaces.desktopCount() >= 4, let fixture = moveFixture {
        ...
        for milliseconds in moveDurations {
            ...
            let focused = focusMoveFixture(fixture)
...
/// Debut with Control-arrow and swipe interception on, and workspace isolation and previews off,
/// which is what the navigation and fullscreen-navigation scenarios exercise.
@MainActor
func launchWithFeatureControls() -> (ready: Bool, settingsBackup: Data?) {
    let settingsBackup = try? Data(contentsOf: settingsFile)
    ...
    clearDiagnosticFile()
    return (waitForDebutReady(launchDebut()), settingsBackup)
}

@MainActor
func finishFeatureControls(settingsBackup: Data?) {
    _ = terminateDebutAndWait()
    if let settingsBackup { try? settingsBackup.write(to: settingsFile, options: .atomic) }
    else { try? FileManager.default.removeItem(at: settingsFile) }
}

// MARK: - Scenario runner

/// Debut's own settings store, which several groups rewrite and restore around their checks.
...
/// desktops (startup discovery) and launch their own instances (the move sweep); a reordered run
/// otherwise handed the next group an emptied instance, which opened an untitled window when
/// activated, or a window parked elsewhere, which changed the drop scenario's source space.
@MainActor
func restoreFixtureWindows() -> Bool {
    let spaces = SpaceService()
    // Only windows on a desktop count. A closed document leaves an ordered-out entry with its old
    // title and no desktop; counting that kept an emptied TextEdit alive, and activating it later
    // opened an untitled window that a fullscreen scenario then took for the fixture.
    func fixtureWindows() -> [(windowID: CGWindowID, pid: pid_t, title: String)] {
        let windows = CGWindowListCopyWindowInfo([.optionAll, .excludeDesktopElements], kCGNullWindowID)
            as? [[String: Any]] ?? []
        var kept: [String: (windowID: CGWindowID, pid: pid_t, title: String)] = [:]
        for window in windows {
            ...
            // One window per document, preferring the one already on the first desktop.
            if let existing = kept[title], spaces.desktopIndex(forWindow: existing.windowID) == 0 || desktop != 0 {
                continue
            }
            kept[title] = (windowID, pid, title)
        }
    ...
    let missing = fixtureDocumentTitles.subtracting(fixtures.map(\.title))
    for title in missing.sorted() {
        let open = Process()
        open.executableURL = URL(fileURLWithPath: "/usr/bin/open")
        // A force-quit TextEdit restores its old windows on the next launch unless told not to.
        open.arguments = [
            "-na", "TextEdit", URL(fileURLWithPath: fixtureDirectory).appendingPathComponent(title).path,
    ...
    if !missing.isEmpty {
        _ = waitFor(timeout: 10) { Set(fixtureWindows().map(\.title)) == fixtureDocumentTitles }
        fixtures = fixtureWindows()
        // A just-launched app's Accessibility server stays silent for up to ~3s, and every group
        // reads the fixture through it; Debut's own focus probe included.
        _ = waitFor(timeout: 10) {
            fixtures.allSatisfy { fixture in
                var windows: CFTypeRef?
                let application = AXUIElementCreateApplication(fixture.pid)
                AXUIElementSetMessagingTimeout(application, 0.5)
                return AXUIElementCopyAttributeValue(application, kAXWindowsAttribute as CFString, &windows) == .success
                    && !((windows as? [AXUIElement]) ?? []).isEmpty
            }
        }
    }
    for fixture in fixtures where spaces.desktopIndex(forWindow: fixture.windowID) != 0 {
        spaces.moveWindow(windowID: fixture.windowID, toDesktop: 0)
    }
    ...
    if !restored || !missing.isEmpty {
        info("Base state: fixtures " + fixtures.map {
            "\($0.title)=\($0.windowID)@\(spaces.desktopIndex(forWindow: $0.windowID).map(String.init) ?? "none")"
        }.joined(separator: " ") + " reopened=\(missing.sorted())")
    }
    return restored
}

/// Returns the session to the state every group starts from: no overlay, overview or held
/// modifier, the fixture windows on the first desktop, and that desktop showing. Debut's own
/// state is each scenario's precondition.
@MainActor
func restoreBaseState() -> Bool {
    postKeyDown(keyCode: CGKeyCode(kVK_Escape))
    postKeyUp(keyCode: CGKeyCode(kVK_Escape))
    postFlagsChanged(flags: [])
    let overviewClosed = dismissSystemWindowOverview()
    let fixturesRestored = restoreFixtureWindows()
    _ = SpaceService().switchToDesktop(index: 0)
    let onFirstDesktop = waitFor { SpaceService().currentDesktopIndex() == 0 }
    wait(0.5)
    if !(overviewClosed && fixturesRestored && onFirstDesktop) {
        info("Base state: overviewClosed=\(overviewClosed) fixturesRestored=\(fixturesRestored) "
            + "firstDesktop=\(onFirstDesktop)")
    }
    return overviewClosed && fixturesRestored && onFirstDesktop
}

/// Puts Debut in the state a scenario expects before it starts.
@MainActor
func satisfyDebutPrecondition(_ precondition: DebutPrecondition) -> Bool {
    let running = !NSRunningApplication.runningApplications(withBundleIdentifier: debutBundleID).isEmpty
    switch precondition {
    case .running where !running:
        clearDiagnosticFile()
        guard waitForDebutReady(launchDebut()) else { return false }
        // Ready is not settled: the legacy order handed these scenarios a Debut that had been
        // running for minutes, with previews for the cards it lays out. Wait for those rather
        // than for a fixed interval.
        _ = waitFor(timeout: 8) {
            let state = readState()
            guard !previewsDisabled, let windows = Int(state["windowsInActiveSpace"] ?? "") else { return true }
            return (Int(state["windowPreviewCount"] ?? "") ?? 0) >= windows
        }
        return true
    case .stopped where running:
        return terminateDebutAndWait()
    default:
        return true
    }
}
...
@MainActor
func runSelectedScenarios(bodies: [String: @MainActor () -> Void]) {
    let catalogIDs = Set(e2eScenarioCatalog.map(\.id))
    let bodyIDs = Set(bodies.keys)
    guard catalogIDs == bodyIDs else {
        fail("Scenario catalog and bodies disagree: catalog-only \(catalogIDs.subtracting(bodyIDs).sorted()), "
    ...
    for scenario in planned {
        ...
        currentGroup = scenario.group.rawValue
        // Groups never inherit each other's leftovers, in the full order or any other.
        let baseRestored = previousGroup == nil || previousGroup == scenario.group || restoreBaseState()
        if !baseRestored || !satisfyDebutPrecondition(scenario.debut) {
            fail("\(scenario.group.rawValue)/\(scenario.id): its starting state could not be established")
            failCount += 1
            totalCount += 1
            recordScenario(scenario, status: "setup_failed")
            continue
        }
        previousGroup = scenario.group
        recordScenario(scenario, status: "ran")
        bodies[scenario.id]?()
    }
    currentScenario = ""
    currentGroup = ""
    for scenario in e2eScenarioCatalog where !planned.contains(where: { $0.id == scenario.id }) {
        recordScenario(scenario, status: "not_selected")
    }
}

// Every scenario body, keyed by its catalog ID; runSelectedScenarios refuses a mismatch.
let scenarioBodies: [String: @MainActor () -> Void] = [
    "baseline": scenario_baseline,
    "system-overviews": scenario_system_overviews,
    "space-switch": scenario_space_switch,
    "overlay-keyboard": scenario_overlay_keyboard,
    "overlay-pointer": scenario_overlay_pointer,
    "window-drop": scenario_window_drop,
    "overlay-keyboard-move": scenario_overlay_keyboard_move,
    "overlay-fullscreen": scenario_overlay_fullscreen,
    "custom-activation": scenario_custom_activation,
...
runSelectedScenarios(bodies: scenarioBodies)
