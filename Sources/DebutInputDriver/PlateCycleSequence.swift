...
public enum PlateCycleKey {
    public static let tab: CGKeyCode = 48
}

/// Builds the stage-cycling gesture: Command and Option stay asserted while Tab is
/// tapped, because releasing Command commits the selection and dropping Option turns
/// each Tab into window cycling instead of stage cycling.
///
...
public enum PlateCycleSequence {
    ...
    private static func tabTap(flags: CGEventFlags) -> [SyntheticKeyEvent] {
        [
            SyntheticKeyEvent(kind: .key(PlateCycleKey.tab, down: true), flags: flags),
            SyntheticKeyEvent(kind: .key(PlateCycleKey.tab, down: false), flags: flags),
        ]
    }
}
