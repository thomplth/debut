...
- **A regular application's plain window is refused another app's fullscreen Space** — Collection behaviour and level do not decide this, and neither did anything Debut changed about the overlay: the app's activation policy does. Measured in the Tart guest against a fullscreen TextEdit, ordering a magenta window front from the background — as `.accessory`, all six variants arrived; as `.regular`, every plain `NSWindow` was absent from the on-screen list and from the pixels at `.statusBar` and `.popUpMenu`, with `.canJoinAllSpaces` alone, plus `.fullScreenAuxiliary`, and plus `.stationary`, while an `NSPanel` with `.nonactivatingPanel` arrived under both policies. So `OverlayWindow` is a panel. Giving Debut a Dock icon made it regular and silently emptied the fullscreen presentation KHA-475 had added, and the E2E check passed throughout because it read `overlayVisible` from the diagnostic state — Debut's own belief — while its screenshot beside it showed bare TextEdit. A presentation check must ask the window server what is on screen.
...
- **A cycle step writes the MRU immediately; deferring the write to a release does not work** — Same-app cycling used to hold its write until the cycle ended, which made every command that reads the MRU depend on being told the cycle was over. Flushing on those commands (KHA-627) was not enough, because an ordinary quick Command-backtick ends with a Command release and nothing else. Measured on the installed build with G1, D1, G2, L1 on one space: two separate quick presses moved the screen G1 → G2 → G1 while Debut's order never left G1 D1 G2, and Command-Tab then offered D1. A press that writes nothing also leaves the cycle open, so the next press re-enters it and steps back — the model stands still while the screen toggles, which is why this reads as a detection failure rather than a write that never happened. What must be frozen is the *walk order* (`backtickCycleWindows`), since re-deriving it from an MRU each step now rewrites would bounce between the two most recent windows and never reach a third; the step itself is taken from the window in front rather than a remembered index, so no ending can desync the cycle from the screen. Ending a cycle then costs nothing. The focus report answering a step's own raise is ignored only while that step is fresh: the app's report regularly names the window the step moved away from, but the same suppression left open would swallow a real click on that app's other window.
- **An MRU rewrite that reports nothing cannot be diagnosed after the fact** — The head decides what every switch offers next, and `recordWindowActivation` moves it more than anything else. It did so silently, so a session that ended in the wrong order left a log showing which commands ran and never the order they left behind — the bug above had to be reproduced live twice, and the first diagnosis from that log was wrong. It reports `window_activation_recorded` with the resulting order, and a cycle step reports `same_app_cycle_stepped`.
