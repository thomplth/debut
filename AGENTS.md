...
- **A held cycle is an unwritten MRU update, so flush it and never discard it** — Same-app cycling holds its MRU write until the cycle ends, so for as long as Command is down the order on screen and the order Debut holds disagree. That shortcut and Command-Tab both live under Command, so reaching another command without lifting it is ordinary use rather than an edge case: every command must commit the pending cycle before it reads or moves the MRU. `setupOverlay`, `quickSwitchToSpace` and `switchToSpace` each cleared `backtickCycleWindows` outright instead, and `handleCmdTabTap` read straight past it. Measured on the installed build with G1, D1, G2, L1 on one space: Command-backtick brought G2 to the front on screen, Debut kept G1 at its head, and Tab under the still-held Command then activated D1 — the second window of the order the user had just left. `commitBacktickCycle` reports `same_app_cycle_committed` because nothing else on that path is durable, and the bug had to be reproduced live before it could be seen at all.
