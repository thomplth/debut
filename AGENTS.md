...
- **A window with no single desktop is not on desktop 1** — Windows assigned to every Space (Finder, on some systems) and windows on a fullscreen Space report no sole index. Absence of an answer means "leave the assignment alone"; reading it as index 0 sweeps those windows onto the first stage and then fights the user every time they move.
